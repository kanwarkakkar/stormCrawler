...
//	private JedisPool pool;
	...
	private static class FetchItemQueue {
		...
		public FetchItemQueue(Config conf, int maxThreads, long crawlDelay, long minCrawlDelay) {
	...
	private static class FetchItemQueues {
		...
		public synchronized FetchItemQueue getFetchItemQueue(String id) {
			...
			if (fiq == null) {
				...
				// initialize queue
				fiq = new FetchItemQueue(conf, customThreadVal, crawlDelay, minCrawlDelay);
				queues.put(id, fiq);
			}
			return fiq;
		}
				...
				try {
					...
					if (rules.getCrawlDelay() > 0) {
						if (rules.getCrawlDelay() > maxCrawlDelay && maxCrawlDelay >= 0) {
							LOG.info("Crawl-Delay for {} too long ({}), skipping", fit.url, rules.getCrawlDelay());
...
//					}
					...
					response.getMetadata().setValue("fetch.loadingTime", Long.toString(timeFetching));
					...
					final Values tupleToSend = new Values(fit.url, response.getMetadata(), status);
					...
					if (status.equals(Status.FETCHED)) {
						if (response.getStatusCode() == 304) {
							...
							collector.emit(fit.t, new Values(fit.url, response.getContent(), response.getMetadata()));
						}
						...
						if (allowRedirs() && StringUtils.isNotBlank(redirection)) {
							emitOutlink(fit.t, URL, redirection, response.getMetadata());
						}
					}
					// error
					else {
	...
	public void execute(Tuple input) {
		...
		do {
			if (this.maxNumberURLsInQueues != -1
					&& (this.activeThreads.get() + this.fetchQueues.inQueues.get()) >= maxNumberURLsInQueues) {
			...
			LOG.info("[Fetcher #{}] Threads : {}\tqueues : {}\tin_queues : {}", taskID, this.activeThreads.get(),
